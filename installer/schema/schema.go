...
type Field struct {
	...
	EnvVar       bool      `yaml:"env_var"`       // if true, setup will offer to set its value and write it to .env
}
...
type Schema struct {
	...
	InstallSubdir string  `yaml:"install_subdir"` // optional subdirectory under install dir (e.g. "wtcommander")
...
func (s *Schema) validate() error {
	...
	for i, field := range s.Fields {
		...
		if field.Type == FieldTypeSelect && len(field.Options) == 0 {
			return fmt.Errorf("field %q is type select but has no options", field.Key)
		}
	}
